...
void dispatch(const double* samples, size_t& now_at_sample, MPI_Status& status, MPI_Comm master_comm)
{
	double send_buffer[1 + nParameters]; // to send sample parameters with sample ID
	if (now_at_sample < nSamples)
	{
		std::copy(&samples[now_at_sample*nParameters],
		     	  &samples[(now_at_sample+1)*nParameters],
				  &send_buffer[1]);
	...
	{	// receive report from consumer:
		...
		switch (status.MPI_TAG)
		{
			...
			case MSG_READY: // consumer reports ready w/o result (during ramp-up)
				dispatch(samples, now_at_sample, status, master_comm); // task consumer or send exit 
				break;
			case MSG_RESULT: // consumer reports ready w/ result
				results[(int) recv_buffer[0]]= recv_buffer[1]; // record result from consumer
				time_counters[status.MPI_SOURCE]+= recv_buffer[2]; // record calculation time from consumer
				dispatch(samples, now_at_sample, status, master_comm);  // task consumer again or send exit 
				break;
