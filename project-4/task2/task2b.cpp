...
upcxx::future<double> dispatch(const upcxx::intrank_t rank,
                               double* samples, const size_t sample_idx)
{   // RPC with lambda function for evaluation
	const int begin= sample_idx * nParameters;
	const int end= begin + nParameters;
	return upcxx::rpc(rank,
                      [](upcxx::view<double> sample_params) -> double {
						  return evaluateSample(const_cast<double*>(&sample_params.front()));
				      }, upcxx::make_view(samples+begin, samples+end));
}


int main(int argc, char* argv[])
{
	upcxx::init(); // they see me rollin'
	...
	{   // master rank prints status message:
		...
		double* samples= initializeSampler(nSamples, nParameters); // populate samples
		double* results= (double*) calloc(nSamples, sizeof(double)); // allocate memory for results
		...
		{
			auto fut_result= dispatch(rank, samples, now_at_sample); // dispatch sample for evaluation
		...
		while (!consumers.empty()) // as long as there are dispatched consumers in the queue
		{
			...
			results[consumer.sample_no]= consumer.my_result.result(); // take calculated result from consumer
			...
			{	// dispatch consumer with next sample and enqueue again		
				consumer.my_result= dispatch(consumer.my_rank, samples, now_at_sample);
